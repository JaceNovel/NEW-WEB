...
type RulesPdfPayload = {
  ...
  function buildPdf(objects: PdfObject[]) {
    ...
    for (let index = 0; index < objects.length; index += 1) {
      const payload = typeof objects[index] === "string" ? Buffer.from(objects[index], "utf8") : objects[index];
      offsets.push(cursor);
  ...
  async function loadPdfAssets() {
    const [leftLogo, rightLogo, markLogo] = await Promise.all([
      readFile("/home/kernelx/NEW-WEB/tournament-platform/public/WhatsApp_Image_2026-03-12_at_12.25.53-removebg-preview.png"),
      readFile("/home/kernelx/NEW-WEB/tournament-platform/public/pp1-removebg-preview (1).png"),
      loadIconAsset(),
    ]);
